...
class StockDataFrame(pd.DataFrame):
    ...
    @staticmethod
    def set_nan(pd_obj, shift, column_name):
        try:
            iter(shift)
            max_shift = max(shift)
            min_shift = min(shift)
            StockDataFrame._set_nan_of_single_shift(pd_obj, max_shift, column_name)
            StockDataFrame._set_nan_of_single_shift(pd_obj, min_shift, column_name)
        except TypeError:
            # shift is not iterable
            StockDataFrame._set_nan_of_single_shift(pd_obj, shift, column_name)

    @staticmethod
    def _set_nan_of_single_shift(pd_obj, shift, column_name):
        val = np.nan
        if shift > 0:
            pd_obj.loc[pd_obj.index[-shift:], column_name] = val
        elif shift < 0:
            pd_obj.loc[pd_obj.index[:-shift], column_name] = val
    ...
    def _get_s(cls, df, column, shifts):
        ...
        StockDataFrame.set_nan(df, shift, shifted_key)
    ...
    def _get_d(df, column, shifts):
        ...
        StockDataFrame.set_nan(df, shift, column_name)
